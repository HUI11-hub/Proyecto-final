...
function renderBoard() {
    ...
    let opsFiltradas = operaciones.filter(op => {
        if (searchCode && !(op.codigo ?? '').toLowerCase().includes(searchCode)) return false;

        if (filterStatus !== 'TODOS' && op.estado !== filterStatus) return false;

        if (horaDesde || horaHasta) {
            const horaOp = (op.horaEstimada ?? '').slice(-8, -3)
                        || (op.horaEstimada ?? '').slice(11, 16);
            if (horaDesde && horaOp < horaDesde) return false;
            if (horaHasta && horaOp > horaHasta) return false;
        }
    ...
    const salidas  = opsFiltradas.filter(op => (op.sentido ?? '').toUpperCase() === 'SALIDA');
    const llegadas = opsFiltradas.filter(op => (op.sentido ?? '').toUpperCase() === 'LLEGADA');
...
function crearTablaHTML(listaOps) {
    ...
    listaOps.forEach(op => {
        const operador = operadores.find(o => o.id == (op.operatorId ?? op.operadorId));
        const punto    = puntos.find(p => p.id    == (op.spotId     ?? op.puntoId));
        const nombreOp = operador ? operador.nombre : '—';
        const codigoPt = punto    ? punto.codigo    : '—';
        const color    = colorEstado[(op.estado ?? '').toUpperCase()] ?? '#eeeeee';
...
document.getElementById('horaDesde')?.addEventListener('change',   renderBoard);
document.getElementById('horaHasta')?.addEventListener('change',   renderBoard);

document.getElementById('btnLimpiarHoras')?.addEventListener('click', () => {
